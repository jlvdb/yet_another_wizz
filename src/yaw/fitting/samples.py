...
class MCSamples(HDFSerializable, FitData):
    ...
    def __init__(
        self, sampler: emcee.EnsembleSampler, parnames: Sequence[str], ndata: int
    ) -> MCSamples:
        ...
        if len(parnames) != ndim:
            raise ValueError("length of 'parnames' does not match feature dimensions")
    ...
    def get_samples(
        self, *, step: _Tslice = slice(None), chain: _Tslice = slice(None)
    ) -> pd.DataFrame:
    ...
    def get_autocorr(self) -> pd.Series:
        ...
        return pd.Series(
            emcee.autocorr.integrated_time(
                samples.reshape((self.nsteps, self.nchains, self.ndim)), quiet=True
            ),
            index=self.parnames,
            name="tau",
        )
    ...
    def get_result(
        self, val_stat: str = "median", err_stat: str = "std", *, sigma: float = 1.0
    ) -> FitResult:
