...
from yaw.core.math import apply_bool_mask_ndim
...
class Optimizer:
    ...
    def set_priors(self, priors: Sequence[Prior] | Mapping[Prior] | None) -> None:
        if priors is None:
            self.priors = None
    ...
    def run_mcmc(
        ...
        *,
        ...
        if p0 is None:
            ...
            p0 = np.column_stack(
                [prior.draw_samples(nwalkers, rng) for prior in self.priors]
            )

        sampler = emcee.EnsembleSampler(nwalkers, self.ndim, self._log_prob_with_blobs)
        ...
        if verbose:
            sys.stdout.write(f"sampling steps: {it:d} of {n_expect:d}: {end_message}\n")
            sys.stdout.flush()
